Cypress.Commands.add('Login', () => {
     ...
     cy.fixture('elementsLogin_SignUp').then((el)=> {
          ...
          cy.get(el.usernameField).should('be.visible').type(randomUsername)
          cy.get(el.emailField).should('be.visible').type(randomEmail)
          cy.get(el.passwordField).should('be.visible').type(el.password)
          cy.wait(1500)
          cy.get('button.btn.btn-lg.btn-primary.pull-xs-right').click()
          cy.get(el.settingsPage).should('be.visible').click();
          cy.get('button.btn.btn-outline-danger').click();
          
          cy.get(el.loginPage).should('be.visible').click()
